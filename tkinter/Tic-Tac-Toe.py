"""
Цей код реалізує гру "Хрестики-нулики" з комп'ютерним супротивником,
використовуючи мінімаксний алгоритм для прийняття рішень комп'ютером.
Гравець (X) і комп'ютер (O) здійснюють свої ходи через клік миші на відповідних клітинках ігрової дошки.
Після закінчення гри (перемога одного з гравців або нічия),
з'являється спливаюче вікно з відповідним повідомленням.
"""
...
# Створюємо екран для гри "Хрестики-нулики"
screen = turtle.Screen()
screen.setup(800, 800)
screen.title("Хрестики-нулики - PythonTurtle.Academy")
...
# Функція для малювання кроку на ігровій дошці
...
# Функція для малювання всієї ігрової дошки
def draw(b):
    draw_board()
    for i in range(3):
        for j in range(3):
...
# Функція для перевірки стану гри (перемога, нічия або гра продовжується)
def gameover(b):
...
# Функція для обробки ходу гравця та ходу комп'ютера
def play(x, y):
    global turn
    if turn == 'x': return
    ...
    r = gameover(b)
    if r == 1:
        screen.textinput("Кінець гри!", "X переміг!")
    elif r == 2:
        screen.textinput("Кінець гри!", "O переміг!")
    elif r == 3:
        screen.textinput("Кінець гри!", "Нічия!")
    if r > 0: turtle.bye()
...
# Ініціалізуємо ігрову дошку та починаємо гру
b = [[0, 0, 0], [0, 0, 0], [0, 0, 0]]
draw(b)
turn = 'x'
screen.onclick(play)

# Функції для розрахунку ходу мінімаксного алгоритму
